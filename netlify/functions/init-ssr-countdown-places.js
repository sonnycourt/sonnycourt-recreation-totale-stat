...
export default async (req, context) => {
    ...
    try {
        ...
        // Vérifier si le token existe déjà (pour éviter de réinitialiser)
        const existingData = await store.get(token);
        if (existingData) {
            const parsed = JSON.parse(existingData);
            console.log('⚠️ Token existe déjà, pas de réinitialisation:', token);
            return new Response(JSON.stringify({
                ...
                message: 'Token déjà initialisé, données existantes conservées'
            }), {
                status: 200,
                headers: {
                    'Access-Control-Allow-Origin': '*',
