...
async function reusableSession(stripe, registration, offer) {
  ...
  return session?.status === 'open'
    ...
    && session.metadata?.payment_plan === offer.paymentPlan
    && Number(session.metadata?.contractual_total_cents || 0) === offer.contractualTotalCents
    ? session
    : null;
}
...
export default async (req) => {
  ...
  try {
    ...
    const checkoutSurface = clean(body.checkout_surface, 40) === 'mc2_session'
      ? 'mc2_session'
      : 'commencer';
    ...
    let offer;
    if (checkoutSurface === 'mc2_session') {
      const plan = mc2SessionPlanConfig(body.plan);
      const prices = await ensureMc2SessionPrices(stripe);
      offer = {
        ...plan,
        entryPriceId: plan.requiresFuturePayments ? prices.monthlyEntry.id : prices.oneTime.id,
      };
    } else {
      ...
      offer = {
        paymentPlan: MC2_PAYMENT_PLAN,
        initialPaymentCents: MC2_ENTRY_PAYMENT_CENTS,
        contractualTotalCents: MC2_CONTRACT_TOTAL_CENTS,
        entryPriceId: MC2_STRIPE_ENTRY_PRICE_ID,
        requiresFuturePayments: true,
      };
    }
    const existing = await reusableSession(stripe, registration, offer);
    if (existing) {
      return json(200, {
        ...
        offer: {
          amount: offer.initialPaymentCents,
          currency: 'eur',
          contractual_total: offer.contractualTotalCents,
          payment_plan: offer.paymentPlan,
        },
      });
    }
    ...
    const metadata = {
      ...
      pay_route: 'pay',
      checkout_id: checkoutSurface === 'mc2_session' ? 'es2-mc2-session' : 'es2-mc2-commencer',
      ...
      payment_plan: offer.paymentPlan,
      contractual_total_cents: String(offer.contractualTotalCents),
      initial_payment_cents: String(offer.initialPaymentCents),
    ...
    const session = await stripe.checkout.sessions.create({
      ui_mode: 'custom',
      mode: 'payment',
      line_items: [{ price: offer.entryPriceId, quantity: 1 }],
      ...
      payment_intent_data: {
        ...(offer.requiresFuturePayments ? { setup_future_usage: 'off_session' } : {}),
      ...
      idempotencyKey: `mc2-checkout:${token}:${offer.paymentPlan}:${clean(body.nonce, 80) || crypto.randomUUID()}`,
    });
