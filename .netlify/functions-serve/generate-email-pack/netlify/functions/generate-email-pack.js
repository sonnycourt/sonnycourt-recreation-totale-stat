...
// netlify/functions/generate-email-pack.cjs
var fetch = require_lib2();
var crypto = require("crypto");
var supabaseUrl = "https://grjbxdraobvqkcdjkvhm.supabase.co";
var supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdyamJ4ZHJhb2J2cWtjZGprdmhtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg0OTM0NTAsImV4cCI6MjA4NDA2OTQ1MH0.RqOx2RfaUf4-JqJpol_TW7h6GD4ExIxJB4Q4jBY5XcQ";
var handler = async (event) => {
  ...
  try {
    console.log("\u{1F4E5} Body re\xE7u de MailerLite:", JSON.stringify(event.body ? JSON.parse(event.body) : {}, null, 2));
    ...
    if (!email) {
      console.error("\u274C Email non trouv\xE9 dans la requ\xEAte");
      return {
        statusCode: 400,
        headers: {
          "Content-Type": "application/json",
    ...
    console.log(`\u{1F50D} Recherche des donn\xE9es du quiz pour l'email: ${email}`);
    ...
    if (!quizDataArray || quizDataArray.length === 0) {
      console.error("\u274C Email non trouv\xE9 dans quiz_responses");
      return {
        statusCode: 404,
        headers: {
          "Content-Type": "application/json",
    ...
    console.log("\u2705 Donn\xE9es du quiz r\xE9cup\xE9r\xE9es:", {
      prenom: quizData.prenom,
      objectif: quizData.objectif,
      situation: quizData.situation,
      token: quizData.token ? "pr\xE9sent" : "absent"
    });
    ...
    if (!atomicUpdateBody || !Array.isArray(atomicUpdateBody) || atomicUpdateBody.length === 0) {
      console.log(`\u23ED\uFE0F SKIP: Email ${emailType} d\xE9j\xE0 en cours/envoy\xE9 pour ${email}`);
      return {
        statusCode: 200,
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
    ...
    lockAcquired = true;
    console.log(`\u{1F512} Verrou acquis pour ${emailType} - ${email}`);
    if (emailType === "24h") {
      ...
      if (!checkData[0]?.email_sent) {
        console.log("\u23F3 ATTENTE: Email 24h demand\xE9 mais email initial pas encore envoy\xE9 pour", email);
        await fetch(
    ...
    if (emailType === "4h") {
      ...
      if (!checkData[0]?.email_24h_sent) {
        console.log("\u23F3 ATTENTE: Email 4h demand\xE9 mais email 24h pas encore envoy\xE9 pour", email);
        await fetch(
          `${supabaseUrl}/rest/v1/quiz_responses?email=eq.${encodeURIComponent(email)}`,
          { method: "PATCH", headers: { "apikey": supabaseAnonKey, "Authorization": `Bearer ${supabaseAnonKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ [sentColumn]: false }) }
        );
    ...
    console.log("\u2705 Ordre v\xE9rifi\xE9, continuation pour", emailType);
    let token = quizData.token;
    if (!token) {
      token = crypto.randomUUID();
      console.log("\u{1F511} Token g\xE9n\xE9r\xE9 (absent dans Supabase):", token);
      ...
      if (!updateResponse.ok) {
        ...
        console.error("\u26A0\uFE0F Erreur lors de la mise \xE0 jour du token:", errorText);
      } else {
        console.log("\u2705 Token stock\xE9 dans Supabase");
      }
    } else {
      console.log("\u2705 Token r\xE9cup\xE9r\xE9 depuis Supabase:", token);
    }
    console.log("\u{1F916} Mod\xE8le LLM s\xE9lectionn\xE9:", model);
    console.log("\u{1F4E7} Type d'email:", emailType);
    let prompt = "";
    if (emailType === "initial") {
      prompt = `Tu es Sonny Court. \xC9cris un email personnel \xE0 ${quizData.prenom || "cette personne"}.

Voici ses r\xE9ponses au quiz :
- Objectif : ${quizData.objectif || "Non sp\xE9cifi\xE9"}
- Situation : ${quizData.situation || "Non sp\xE9cifi\xE9"}
- Fiert\xE9 : ${quizData.fierte || "Non sp\xE9cifi\xE9"}
- R\xEAve : ${quizData.reve || "Non sp\xE9cifi\xE9"}
- Souffrance : ${quizData.souffrance || "Non sp\xE9cifi\xE9"}

AVANT TOUT : Si les r\xE9ponses sont du charabia, des mots random, ou clairement pas s\xE9rieuses \u2192 retourne uniquement : SKIP
Si les r\xE9ponses sont courtes mais coh\xE9rentes \u2192 c'est OK, g\xE9n\xE8re l'email.

OBJECTIF DE L'EMAIL :
Que la personne se dise en lisant : 'Putain, il me parle \xE0 MOI. Il m'a comprise. Et il a la solution.'

Chaque phrase doit servir l'un de ces trois piliers :
1. CONNEXION \u2192 Elle se sent vue et comprise (utilise SES mots, SES d\xE9tails)
2. ESPOIR \u2192 Le changement est possible pour elle sp\xE9cifiquement
3. URGENCE \u2192 Chaque jour sans action renforce ses blocages

Si une phrase ne sert aucun de ces piliers, supprime-la.

M\xC9CANISMES \xC0 SUIVRE (dans cet ordre) :
1. Accroche \u2192 quelque chose de sp\xE9cifique dans ses r\xE9ponses qui t'a marqu\xE9
2. Valorise sa fiert\xE9 \u2192 montre que c'est rare/courageux
3. Connecte \xE0 son r\xEAve \u2192 c'est l\xE9gitime, atteignable
4. Empathie sur sa souffrance \u2192 tu comprends, c'est dur
5. Le vrai probl\xE8me \u2192 programmes subconscients, pas un d\xE9faut personnel
6. Transition naturelle \u2192 'Du coup j'ai r\xE9fl\xE9chi \xE0 ce qui pourrait vraiment t'aider...'
7. Pack Complet = parcours strat\xE9gique, pas une compilation en vrac
8. Lien vers l'offre
9. Signature
10. PS qui cr\xE9e l'urgence en reprenant un \xE9l\xE9ment de sa souffrance

IMPORTANT - POSITIONNEMENT DU PACK :

Le Pack Complet est une m\xE9thode de REPROGRAMMATION DU SUBCONSCIENT.
Ce n'est PAS une formation sp\xE9cialis\xE9e sur l'objectif de la personne (pas une formation business, pas une formation s\xE9duction, pas une formation confiance).

Le principe :
- Le M\xC9CANISME est toujours le m\xEAme : reprogrammer le subconscient
- L'OBJECTIF de la personne est le R\xC9SULTAT que ce m\xE9canisme permet d'atteindre

Donc : "Reprogrammer ce qui te bloque pour [son objectif]"
Jamais : "T'apprendre comment [son objectif]"

L'objectif n'est pas enseign\xE9, il est d\xE9bloqu\xE9.

\xC9L\xC9MENTS OBLIGATOIRES :
...
- Dire 'cette offre' (pas 'l'offre') pour renforcer le c\xF4t\xE9 unique

FORMATAGE HTML DE L'EMAIL :
- Taille de police : 16px minimum pour le body
- Interligne : line-height 1.7 pour une lecture agr\xE9able
- Espacement : marge entre chaque paragraphe (margin-bottom: 16px sur les <p>)
- Mots/phrases cl\xE9s en <strong> pour attirer l'\u0153il (2-3 par email max, pas plus)
- Lien CTA bien visible : couleur #4D97FE, soulign\xE9
...
<div style="font-family: Arial, sans-serif; font-size: 16px; line-height: 1.7; color: #333;">
  ...
  <p style="margin-bottom: 16px;">Paragraphe avec <strong>mots cl\xE9s</strong> en gras...</p>
  ...
  <p style="margin-bottom: 16px;"><a href="https://sonnycourt.com/pack-complet/?token=${token}" style="color: #4D97FE; text-decoration: underline;">Cette offre est disponible 48h seulement ici</a> avec 70% de r\xE9duction.</p>
...
- Ne pas \xE9crire "497\u20AC au lieu de 1682\u20AC"
- Juste dire "cette offre \xE0 -70%" ou "avec 70% de r\xE9duction"

URGENCE 48H - JUSTIFIER NATURELLEMENT :
- Ne pas juste dire "disponible 48h"
- Expliquer pourquoi : "Je te laisse 48h pour y r\xE9fl\xE9chir, pas plus - parce que dans mon exp\xE9rience, au-del\xE0 de ce d\xE9lai, on remet \xE0 plus tard et plus tard devient jamais."
- Ou : "48h, c'est le temps que je te donne pour d\xE9cider. Assez pour r\xE9fl\xE9chir, pas assez pour procrastiner."
- L'IA peut varier la formulation mais doit toujours justifier le d\xE9lai

LIBERT\xC9 TOTALE SUR :
- Les formulations exactes
- Le style d'accroche (varie \xE0 chaque fois)
- Le ton (adapte-le \xE0 ce que la personne a \xE9crit)
- La longueur des paragraphes

NE JAMAIS utiliser deux fois la m\xEAme accroche ou la m\xEAme structure de phrase. Sois cr\xE9atif, authentique, comme si tu \xE9crivais vraiment \xE0 cette personne.

Format de r\xE9ponse :
SUBJECT: [objet de l'email - doit \xEAtre personnel et intrigant - TEXTE BRUT UNIQUEMENT, pas de ** ou __ ou * ou _, pas de formatage Markdown]
BODY: [corps de l'email incluant le PS \xE0 la fin]`;
    } else if (emailType === "24h") {
      prompt = `Tu es Sonny Court. Email de rappel court.

R\xC8GLE CRITIQUE : Tu dois retourner EXACTEMENT ce format avec SUBJECT: sur une ligne et BODY: sur une ligne s\xE9par\xE9e.
...
<p style="margin-top: 16px; font-style: italic;">PS : [Une phrase cr\xE9ative et \xE9motionnelle qui utilise le r\xEAve pour cr\xE9er l'envie d'agir. R\xEAve du user : ${quizData.reve}. Ne PAS juste r\xE9p\xE9ter mot pour mot, mais UTILISER les mots pour raviver le d\xE9sir. Exemple : "Cette libert\xE9 dont tu r\xEAves, elle commence par une d\xE9cision."]</p>
</div>

IMPORTANT : 
- SUBJECT: doit \xEAtre sur sa propre ligne, suivi d'un saut de ligne
- BODY: doit \xEAtre sur sa propre ligne, suivi du HTML
...
- Formatage HTML strict : <div style="font-family: Arial, sans-serif; font-size: 16px; line-height: 1.7; color: #333;"> pour le wrapper et <p style="margin-bottom: 8px;"> pour chaque paragraphe`;
      ...
      prompt = `Tu es Sonny Court. Dernier rappel \xE0 ${quizData.prenom}.

L'offre expire dans 4h.

R\xC8GLE CRITIQUE : Tu dois retourner EXACTEMENT ce format avec SUBJECT: sur une ligne et BODY: sur une ligne s\xE9par\xE9e.

SUBJECT: Derni\xE8re chance

BODY:
<div style="font-family: Arial, sans-serif; font-size: 16px; line-height: 1.7; color: #333;">
<p style="margin-bottom: 8px;">Hello ${quizData.prenom},</p>
<p style="margin-bottom: 8px;">L'offre expire dans 4h.</p>
<p style="margin-bottom: 8px;"><a href='https://sonnycourt.com/pack-complet/?token=${token}' style='color: #4D97FE; text-decoration: underline;'>Derni\xE8re chance</a></p>
<p style="margin-bottom: 8px;">Sonny</p>
<p style="margin-top: 16px; font-style: italic;">PS : [Une phrase cr\xE9ative et \xE9motionnelle qui utilise la souffrance pour cr\xE9er l'urgence. Souffrance du user : ${quizData.souffrance}. Ne PAS juste r\xE9p\xE9ter mot pour mot, mais UTILISER les mots pour cr\xE9er l'envie d'agir maintenant. Exemple : "Dans 4h, soit tu restes avec ce manque de confiance, soit tu d\xE9cides que \xE7a change."]</p>
</div>
...
BODY: [corps de l'email incluant le PS \xE0 la fin]`;
    ...
    if (model !== "sonnet") {
      try {
        ...
        if (!deepseekApiKey) {
          throw new Error("DEEPSEEK_API_KEY non d\xE9finie");
        }
        console.log("\u{1F916} Appel \xE0 l'API DeepSeek...");
        ...
        console.log("\u2705 R\xE9ponse DeepSeek re\xE7ue");
        console.log("R\xE9ponse DeepSeek brute:", JSON.stringify(deepseekData, null, 2));
      ...
      if (!anthropicApiKey) {
        console.error("\u274C ANTHROPIC_API_KEY_EMAIL_PACK non d\xE9finie");
        return {
          statusCode: 500,
          headers: {
            "Content-Type": "application/json",
      ...
      console.log("\u{1F916} Appel \xE0 l'API Anthropic (Claude Sonnet)...");
      ...
      console.log("\u2705 R\xE9ponse Anthropic re\xE7ue");
      console.log("R\xE9ponse Anthropic brute:", JSON.stringify(anthropicData, null, 2));
      content = anthropicData.content?.[0]?.text || "";
    }
    console.log("\u{1F916} Mod\xE8le utilis\xE9:", usedModel);
    console.log("\u{1F4C4} Contenu brut extrait:", content);
    console.log("Contenu brut extrait:", content);
    if (content.trim() === "SKIP" || content.trim().startsWith("SKIP")) {
      console.log("\u26A0\uFE0F R\xE9ponses jug\xE9es inexploitables par Claude");
      return {
        statusCode: 200,
        headers: {
          "Content-Type": "application/json",
    ...
    const subjectColumn = emailType === "initial" ? "email_initial_subject" : emailType === "24h" ? "email_24h_subject" : "email_4h_subject";
    const bodyColumn = emailType === "initial" ? "email_initial_body" : emailType === "24h" ? "email_24h_body" : "email_4h_body";
    try {
      ...
      if (saveEmailRes.ok) {
        console.log("\u{1F4BE} Email sauvegard\xE9 dans Supabase:", emailType);
      } else {
    ...
    if (emailType === "24h" || emailType === "4h") {
      ...
      console.log("\u{1F9F9} Body nettoy\xE9 des <br>");
    }
    const footer = `
<p style="margin-top: 32px; font-size: 12px; color: #666; text-align: left;">
  <a href="https://sonnycourt.com/.netlify/functions/unsubscribe?email=${email}" style="color: #666;">Se d\xE9sinscrire</a>
</p>
`;
    const bodyWithFooter = htmlBody + footer;
    const result = {
      subject: subject || "Email personnalis\xE9",
      body: bodyWithFooter,
      token
    };
    ...
    if (!listmonkUser || !listmonkPass) {
      console.error("\u274C LISTMONK_USER ou LISTMONK_PASS non d\xE9finie");
      return {
        statusCode: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    try {
      console.log("\u{1F4E8} Envoi de l'email via ListMonk...");
      console.log("\u{1F517} LISTMONK_URL:", listmonkUrl);
      console.log("\u{1F464} LISTMONK_USER pr\xE9sent:", !!listmonkUser);
      console.log("\u{1F4E7} Email destinataire:", email);
      console.log("\u{1F4E7} Subject:", result.subject);
      console.log("\u{1F464} Pr\xE9nom:", quizData.prenom || "Non sp\xE9cifi\xE9");
      console.log("\u{1F4DD} Body HTML final (premiers 200 caract\xE8res):", result.body.substring(0, 200));
      const authHeader = "Basic " + Buffer.from(`${listmonkUser}:${listmonkPass}`).toString("base64");
      console.log("\u{1F464} Cr\xE9ation/mise \xE0 jour de l'abonn\xE9 dans ListMonk...");
      ...
      if (createSubscriber.ok) {
        console.log("\u2705 Abonn\xE9 cr\xE9\xE9/mis \xE0 jour dans ListMonk");
      } else {
        console.log("\u26A0\uFE0F Erreur cr\xE9ation/mise \xE0 jour abonn\xE9 ListMonk (continuit\xE9 quand m\xEAme):", createSubscriber.status, createSubscriberText);
      }
      console.log("\u{1F6A8} ENVOI EMAIL - " + Date.now() + " - " + email + " - " + emailType);
      ...
      console.log("\u2705 Email envoy\xE9 via ListMonk");
      console.log("\u{1F4E7} Response:", listmonkResponseText);
    } catch (listmonkError) {
    ...
    console.log("\u2705 Traitement termin\xE9 avec succ\xE8s pour:", email);
    return {
      statusCode: 200,
      headers: {
        "Content-Type": "application/json",
...
//# sourceMappingURL=generate-email-pack.js.map
