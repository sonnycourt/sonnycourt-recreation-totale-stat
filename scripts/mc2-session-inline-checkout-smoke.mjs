...
assert.match(page, /preview-payment\.is-confirmed/);
assert.match(page, /Paiement confirmé/);
assert.match(page, /\/commencer\/succes\/\?provider=spiffy&t=/);
assert.match(page, /Ton accès est prêt\. Tu peux continuer la masterclass/);
assert.match(page, /id="dev-payment-success"[^>]*>Après achat ✓<\/button>/);
assert.match(page, /window\.__mc2PreviewPaymentSuccess/);
assert.match(page, /showPaymentSuccess\(\{ payment_status: 'paid' \}, \{ simulated: true \}\)/);
assert.match(page, /if \(options\.simulated !== true\) \{\s*trackWebinaireEvent/);
assert.doesNotMatch(page, /handleSpiffyOrderSuccess[\s\S]{0,1800}\.pause\(\)/);
