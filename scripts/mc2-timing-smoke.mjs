...
assert.equal(MC2_LIVE_VIDEO_DURATION_SECONDS, 7_103);
assert.equal(MC2_SESSION_DURATION_SECONDS, 6_203);
assert.equal(MC2_SESSION_DURATION_MS, 6_203_000);
assert.equal(MC2_LIVE_CTA_SECONDS, 97 * 60 + 28);
assert.equal(MC2_REPLAY_CTA_SECONDS, 77 * 60 + 28);
assert.equal(mc2SessionDurationMs(), MC2_SESSION_DURATION_MS);

const sessionStart = '2026-08-26T18:00:00.000Z';
assert.equal(mc2SessionEndsAt(sessionStart).toISOString(), '2026-08-26T19:43:23.000Z');
assert.equal(mc2SessionEndsAtIso(sessionStart), '2026-08-26T19:43:23.000Z');
...
assert.equal(selection.sessionEndsAt.toISOString(), '2026-08-26T19:43:23.000Z');
...
console.log(JSON.stringify({
  canonical_live_end_014323_after_session_start: 'ok',
