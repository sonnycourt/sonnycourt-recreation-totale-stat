...
import { validateCommand, validateDeletion, validContactTime, contactStep, plusDays, presentCase, formatRegistrationTime, contactTimer, firstSuccessfulContact, CONTACT_TARGET_MS } from '../netlify/functions/lib/onboarding-domain.mjs';
...
ok(contactTimer(clockBase,clockHour(12)).value==='36:00:00','48h countdown starts from enrollment, not opening CRM');
ok(contactTimer(clockBase,clockHour(49)).state==='overdue'&&contactTimer(clockBase,clockHour(49)).value==='+ 01:00:00','overdue countdown remains visible');
ok(contactTimer({...clockBase,first_successful_contact_at:new Date(clockHour(24)).toISOString()},clockHour(72)).state==='within','successful contact freezes on time even days later');
ok(contactTimer({...clockBase,first_successful_contact_at:new Date(clockHour(48)).toISOString()},clockHour(72)).state==='within','exactly 48h is on time');
ok(contactTimer({...clockBase,first_successful_contact_at:new Date(clockHour(48)+1).toISOString()},clockHour(72)).state==='late','48h plus 1ms is late');
ok(contactTimer({...clockBase,status:'done'},clockHour(72)).state==='overdue','manual status does not invent a successful contact');
ok(contactTimer({...clockBase,contact_timing_known:false},clockHour(72)).state==='unknown','unavailable history not mislabeled as no contact');
ok(contactTimer({...clockBase,first_successful_contact_at:new Date(clockHour(-1)).toISOString()},clockHour(72)).state==='unknown','contact predating enrollment needs correction');
ok(contactTimer({...clockBase,first_successful_contact_at:new Date(clockHour(90)).toISOString()},clockHour(72)).state==='unknown','future successful contact not accepted as proof');
ok(contactTimer({...clockBase,purchased_at:'2026-10-24T22:00:00Z'},Date.parse('2026-10-26T22:00:00Z')).state==='overdue'&&CONTACT_TARGET_MS===172800000,'48 elapsed hours even through DST');
...
ok(firstSuccessfulContact(clockEvents)===new Date(clockHour(20)).toISOString(),'only actual non-deleted successful contacts stop the clock');
